...
// ─── Slide thumbnail (scaled iframe) ─────────────────────────────────────────

function SlideThumb({ session }: { session: SessionSummary }) {
...
// Mini thumbnail for list view (fixed 80×45px — no ResizeObserver needed)
function MiniThumb({ session }: { session: SessionSummary }) {
  const ref = useRef<HTMLDivElement>(null);
      ...
      >
        ...
        <p className="text-[11.5px] mt-0.5" style={{ color: "var(--text3)" }}>
          {session.slideCount > 0
            ? `${session.slideCount} slide${session.slideCount !== 1 ? "s" : ""} · `
            : ""}
  ...
  return (
    <div className="flex flex-col items-center justify-center h-full gap-4 pb-16">
      ...
      <div style={{ position: "relative", width: 130, height: 78 }}>
        {[
          { top: 12, left: 8, w: 114, h: 64 },
          { top: 6, left: 4, w: 122, h: 68 },
          { top: 0, left: 0, w: 130, h: 74 },
        ].map((s, i) => (
          <div
            key={i}
            style={{
...
export default function PresentationsPage() {
  ...
  const load = useCallback(async () => {
    setLoading(true);
    try {
      const res = await fetch("/api/sessions?type=slides");
  ...
  async function handleLoadMore() {
    ...
    setLoadingMore(true);
    try {
      const res = await fetch(`/api/sessions?type=slides&cursor=${nextCursor}`);
      const data = await res.json();
