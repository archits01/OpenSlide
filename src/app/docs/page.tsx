...
function DocThumb({ session }: { session: SessionSummary }) {
  ...
  const html = buildSlideHtml(
    {
      ...
      index: 0,
      title: "",
      content: session.firstSlide.content,
      layout: "content" as const,
    },
    (session.firstSlide.theme as ThemeName) || "minimal",
    null,
    true
  );
...
function MiniDocThumb({ session }: { session: SessionSummary }) {
  if (!session.firstSlide) {
    return (
      <div
        style={{
      ...
      >
        ...
        <p className="text-[11.5px] mt-0.5" style={{ color: "var(--text3)" }}>
          {session.slideCount > 0
            ? `${session.slideCount} page${session.slideCount !== 1 ? "s" : ""} · `
            : ""}
...
function EmptyState({ onNew }: { onNew: () => void }) {
  return (
    <div className="flex flex-col items-center justify-center h-full gap-4 pb-16">
      {/* Stacked docs illustration */}
      <div style={{ position: "relative", width: 100, height: 110 }}>
        {[
          { top: 16, left: 10, w: 80, h: 100 },
          { top: 8, left: 5, w: 90, h: 104 },
          { top: 0, left: 0, w: 100, h: 108 },
        ].map((s, i) => (
          <div
            key={i}
            style={{
          ...
          >
            {i === 2 && (
              <>
                <div style={{ width: 42, height: 3, borderRadius: 999, background: "var(--border-hover)" }} />
                <div style={{ width: 60, height: 2, borderRadius: 999, background: "var(--border)" }} />
                <div style={{ width: 54, height: 2, borderRadius: 999, background: "var(--border)" }} />
                <div style={{ width: 48, height: 2, borderRadius: 999, background: "var(--border)" }} />
                <div style={{ width: 36, height: 2, borderRadius: 999, background: "var(--border)" }} />
              </>
            )}
          </div>
        ))}
...
export default function DocsPage() {
  ...
  const load = useCallback(async () => {
    setLoading(true);
    try {
      const res = await fetch("/api/sessions?type=docs");
  ...
  async function handleLoadMore() {
    ...
    setLoadingMore(true);
    try {
      const res = await fetch(`/api/sessions?type=docs&cursor=${nextCursor}`);
      const data = await res.json();
  ...
  function handleOpen(id: string) {
    // Touch lastOpenedAt so this document sorts first on next visit
    fetch("/api/sessions", {
      method: "PATCH",
