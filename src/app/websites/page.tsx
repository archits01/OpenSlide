...
// Globe/meridian icon — matches the Website pill in InputToolbar + the sidebar nav
function GlobeIcon({ size = 28, color = "rgba(255,255,255,0.3)" }: { size?: number; color?: string }) {
  return (
...
// ─── Website thumbnail (preview screenshot or gradient fallback) ─────────────

function WebsiteThumb({ session }: { session: SessionSummary }) {
  if (!session.previewScreenshotUrl) {
    return (
      <div
        style={{
          width: "100%",
  ...
  return (
    ...
    >
      ...
      <img
        src={session.previewScreenshotUrl}
        alt=""
        style={{
          display: "block",
          width: "100%",
...
function MiniWebsiteThumb({ session }: { session: SessionSummary }) {
  if (!session.previewScreenshotUrl) {
    return (
      <div
        style={{
...
function ListRow({
  ...
  return (
    ...
    >
      <MiniWebsiteThumb session={session} />
...
function EmptyState({ onNew }: { onNew: () => void }) {
  return (
    <div className="flex flex-col items-center justify-center h-full gap-4 pb-16">
      {/* Stacked browser-window illustration */}
      <div style={{ position: "relative", width: 130, height: 82 }}>
        {[
          { top: 12, left: 8, w: 114, h: 68 },
          { top: 6, left: 4, w: 122, h: 72 },
          { top: 0, left: 0, w: 130, h: 78 },
        ].map((s, i) => (
          <div
            key={i}
            style={{
            ...
            {i === 2 && (
              <>
                ...
                <div style={{ padding: "10px 12px", display: "flex", flexDirection: "column", gap: 6 }}>
                  <div style={{ width: 52, height: 3, borderRadius: 999, background: "var(--border-hover)" }} />
                  <div style={{ width: 34, height: 2, borderRadius: 999, background: "var(--border)" }} />
                  <div style={{ width: 42, height: 2, borderRadius: 999, background: "var(--border)" }} />
                </div>
              </>
            )}
          </div>
...
export default function WebsitesPage() {
  ...
  const load = useCallback(async () => {
    setLoading(true);
    try {
      const res = await fetch("/api/sessions?type=website");
  ...
  async function handleLoadMore() {
    ...
    setLoadingMore(true);
    try {
      const res = await fetch(`/api/sessions?type=website&cursor=${nextCursor}`);
      const data = await res.json();
  ...
  function handleOpen(id: string) {
    // Touch lastOpenedAt so this session sorts first on next visit
    fetch("/api/sessions", {
      method: "PATCH",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ id }),
    }).catch(() => {});
    // Every session on this page is a website session — use a full page load
    // so the editor inherits cross-origin isolation (SharedArrayBuffer → live
    // preview works without a manual reload).
